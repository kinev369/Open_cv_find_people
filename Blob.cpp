...
Blob::Blob(std::vector<cv::Point> _contour) {
	...
	currentCenter.x = (currentBoundingRect.x + currentBoundingRect.x + currentBoundingRect.width) / 2;// координаты центра 
	currentCenter.y = (currentBoundingRect.y + currentBoundingRect.y + currentBoundingRect.height) / 2;// кооридинаты центра 

	centerPositions.push_back(currentCenter);

	dblCurrentDiagonalSize = sqrt(pow(currentBoundingRect.width, 2) + pow(currentBoundingRect.height, 2));//длина диагонали

	dblCurrentAspectRatio = (float)currentBoundingRect.width / (float)currentBoundingRect.height;//критерий для определения машины как объекта
